  ...
  useEffect(() => {
    ...
    if (token) {
     ...
     setState(decodedState);
      ...
      setWard(decodedWard);
      fetch(`http://localhost:5000/api/leaderboard?state=${decodedState}&district=${decodedDistrict}&ward=${decodedWard}`)
        .then(res => res.json())
        .then(data => {
          setLeaderboard(data.leaderboard || []);
          setTotalUsersInWard(data.totalUsers || 0);

          const wardIndex = data.leaderboard.findIndex(entry => entry.ward === decodedWard);
